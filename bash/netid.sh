...
case "$1" in
    -v)
        verbose="yes"
        echo "Verbose is now enabled"
        ;;
# Display help message and exit
    -* | --help)
        echo "Help Options"
        echo "Hey! You can use this script as: $(basename "$0") [-v] [interface]"
        exit 1
        ;;
# Check if interface argument is already set, if not, set it to the current argument
    * )
        if [ -z "$interface" ]; then
            all_interface="$1"
        else
# Too many arguments, display error message and exit
            echo "lots of arguments given: $@"
            echo "You can use this script as: $(basename "$0") [-v] [interface]"
            exit 1
        fi
        ;;
  esac
...
if [ -n "$all_interface" ];then
	ip addr | grep $all_interface >/dev/null
	if [ $? -eq 0 ]; then
		echo "You have this interface on your Device."
		interface=$all_interface
	else
		echo "This is the list of all interface present in your device. So try running the script again with valid interface"
...
#####
#TASK 2
#Interface without loopback

echo "NOW LOOPING through all interfaces"

lists=$(ip -br addr | awk '{print $1}' | grep -v "lo")

for interface in $lists; do
	[ "$verbose" = "yes" ] && echo "Getting IPV4 network block info and name for interface $interface"
	ipv4_address=$(ip a s $interface|awk -F '[/ ]+' '/inet /{print $3}')
	ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')
	network_address=$(ip route list dev $interface scope link|cut -d ' ' -f 1)
